    ...
    <section className="ic-page">
      <header className="ic-page-header">
        <div className="ic-page-title-group">
          ...
          <h1 className="ic-page-title">Field #{data.id}</h1>
        </div>
        <div className="ic-page-actions">
          <button type="button" className="ic-btn ic-btn-primary"
            onClick={() => navigate(`/settings/field/${data.id}/edit`)}>Edit</button>
      ...
      <dl className="profile-grid">
        ...
        <dt>Required</dt><dd>{data.required ? 'Yes' : 'No'}</dd>
        <dt>Default Value</dt><dd>{data.defaultValue ?? '—'}</dd>
        <dt>Type</dt><dd>{data.type}</dd>
      </dl>
      <div style={{ marginTop: 12 }}>
        <strong>Properties</strong>
        <PropertiesDisplay type={data.type} properties={data.properties} />
      </div>
      ...
      {dialogOpen && (
        <ConfirmDeleteDialog busy={deleting} message={`Delete Field #${data.id}?`}
