...
import { useIssueCategories } from './use-issue-categories';

export function IssueCategoryListPage() {
  const navigate = useNavigate();
  const apiFetch = useApiFetch();
  const { state, reload } = useIssueCategories();
  ...
  const rows: IssueCategoryDto[] = state.kind === 'ready' ? state.data : [];
  ...
  const onAdd = useCallback(() => {
    navigate('/settings/issue-category/new');
  }, [navigate]);
  ...
  return (
    <section className="ic-page">
      <header className="ic-page-header">
        <h1 className="ic-page-title">All Issue Categories</h1>
