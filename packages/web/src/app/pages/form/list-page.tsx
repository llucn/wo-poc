...
export function FormListPage() {
  ...
  const { state, reload } = useForms();
  const [selected, setSelected] = useState<ReadonlySet<number>>(() => new Set());
  const [dialogOpen, setDialogOpen] = useState(false);
  const [deleting, setDeleting] = useState(false);
  const [deleteError, setDeleteError] = useState<string | null>(null);

  const rows: FormDto[] = state.kind === 'ready' ? state.data : [];
  ...
  const tableBody = useMemo(() => {
    ...
    if (rows.length === 0) {
      return <tr><td className="ic-table-empty" colSpan={5}>No forms yet.</td></tr>;
    }
    return rows.map((row) => (
      <tr key={row.id}>
        <td className="ic-col-check">
          <input type="checkbox" aria-label={`Select form ${row.name}`}
            checked={selected.has(row.id)} onChange={() => toggleRow(row.id)} />
        </td>
        <td className="ic-col-id">#{row.id}</td>
        <td><Link to={`/settings/form/${row.id}`}>{row.name}</Link></td>
        <td>{row.description && row.description.length > 100 ? row.description.slice(0, 100) + '...' : row.description}</td>
        <td><Link to={`/settings/form/${row.id}/design?from=list`}>Design</Link></td>
      </tr>
    ));
  ...
  return (
    <section className="ic-page">
      <header className="ic-page-header">
        <h1 className="ic-page-title">All Forms</h1>
        <div className="ic-page-actions">
          <button type="button" className="ic-btn ic-btn-primary"
            onClick={() => navigate('/settings/form/new')}>+ Add</button>
      ...
      {dialogOpen && (
        <ConfirmDeleteDialog busy={deleting} message="Delete Forms?"
